...
# When $d$ changes by a small amount, \texttt{DiffOpt.forward\_differentiate!} solves a linear system capturing the KKT conditions, revealing how $g_1^*$, $g_2^*$, and $\phi^*$ shift with respect to $d$. These sensitivities are critical for power systems operators to understand how different plants ramp up or down as demand fluctuates.
...
for (k, d_val) in enumerate(d_range)
    ...
    @objective(model, Min,                           # cost
        sum(c[i]*g[i] + c2[i]*g[i]^2 for i=1:2) + cφ*φ)
    ...
    g1[k] = value(g[1]);  g2[k] = value(g[2]);  _φ[k] = value(φ)
...
# ----- fixed stacked-area dispatch plot -----
plt_dispatch = plot(d_range, g1;                    # lower layer
    fillrange = 0,                                  # fill down to zero
    label = "g₁",
    xlabel = "Demand (MWh)", ylabel = "g (MWh)",
    left_margin = 5Measures.mm,
    bottom_margin = 5Measures.mm,
    title = "Optimal dispatch");

plot!(plt_dispatch, d_range, g1 .+ g2;              # upper layer
    fillrange = g1,                                 # fill down to g₁
    label = "g₂");

plot!(plt_dispatch, d_range, _φ;                     # unmet demand as a line
    lw = 2, c = :red, label = "φ (unserved)");

capacity_limit = d_range[findfirst(g2 .== 80.0)]

vline!(plt_dispatch, [capacity_limit, sum(G)]; l = :dash, c = :black,
       label = "capacity limit");

# (b) marginal cost λ(d)
plt_mc = plot(d_range, dJ_dd, lw=2, c=:black,
    xlabel = "Demand (MWh)",
    ylabel = "λ (\$/MWh)",
...
plt_sens = plot(d_range, dg1_dd, lw=2,
    xlabel = "Demand (MWh)",
...
# demands ranging from \(\approx 0\) to \SI{300}{\mega\watt\hour}.  The vertical
# dashed lines mark when the individual plant capacities are reached---$g_2$ reaches it's capacity at
# \SI{148}{\mega\watt\hour} and $g_1$ at \SI{230}{\mega\watt\hour}.
