...
function MOI.get(
    ...
    if MOI.Bridges.is_bridged(b, obj_attr)
        return MOI.get(MOI.Bridges.recursive_model(b), attr, MOI.Bridges.bridge(b, obj_attr))
    else
...
function _copy_dual(dest::MOI.ModelLike, src::MOI.ModelLike, index_map)
    ...
    for (F, S) in MOI.get(dest, MOI.ListOfConstraintTypesPresent())
        _copy_constraint_start(dest, src, index_map.con_map[F, S], MOI.ConstraintPrimalStart(), MOI.ConstraintPrimal())
        _copy_constraint_start(dest, src, index_map.con_map[F, S], MOI.ConstraintDualStart(), MOI.ConstraintDual())
    end
    MOI.set(dest, ObjectiveDualStart(), -1.0)
    MOI.set(dest, ObjectiveSlackGapPrimalStart(), 0.0)
end
